...
$sql = "DELETE FROM `gatherings` WHERE `id`=?;";
$stmt = $mysqlConnection->getMysqli()->prepare($sql);

$stmt->bind_param("i", $_POST['gth-frm-id']);
...
if(!$result){
    ...
    $stmt->close();
    $mysqlConnection->disconnect();
    $_SESSION['success'] = "The gathering was removed successfully";
    redirect(null);
}

function redirect($error, $address = "../../admin/pages/gatherings.php", $replace = true, $code = 303){
